...
describe('Eastmoney - A Share Kline', () => {
  describe('getHistoryKline', () => {
    it('should return 日K线数据', async () => {
      const res = await sdk.kline.cn('000001', {
        startDate: '20241201',
        endDate: '20241217',
      });
    ...
    it('should return 周K线数据', async () => {
      const res = await sdk.kline.cn('sz000858', {
        period: 'weekly',
        startDate: '20241101',
        endDate: '20241231',
      });
    ...
    it('should support different adjust types', async () => {
      const [hfq, qfq, noAdj] = await Promise.all([
        sdk.kline.cn('000001', { adjust: 'hfq', startDate: '20241201', endDate: '20241217' }),
        sdk.kline.cn('000001', { adjust: 'qfq', startDate: '20241201', endDate: '20241217' }),
        sdk.kline.cn('000001', { adjust: '', startDate: '20241201', endDate: '20241217' }),
      ]);
    ...
    it('should support sh prefix', async () => {
      const res = await sdk.kline.cn('sh600000', {
        startDate: '20241201',
        endDate: '20241217',
      });
      expect(res.length).toBeGreaterThan(0);
    ...
    it('should support bj prefix', async () => {
      const res = await sdk.kline.cn('bj430047', {
        startDate: '20241201',
        endDate: '20241217',
      });
    ...
    it('should handle 6-digit code starting with 6', async () => {
      const res = await sdk.kline.cn('600000', {
        startDate: '20241201',
        endDate: '20241217',
      });
      expect(res.length).toBeGreaterThan(0);
    ...
    it('should throw for invalid period', async () => {
      await expect(
        sdk.kline.cn('000001', { period: 'yearly' as any })
      ).rejects.toThrow(/period/i);
    });

    it('should throw for invalid adjust', async () => {
      await expect(
        sdk.kline.cn('000001', { adjust: 'bad' as any })
  ...
  describe('getMinuteKline', () => {
    it('should return 分时数据 (period=1)', async () => {
      const res = await sdk.kline.cnMinute('000001', { period: '1' });
      expect(res.length).toBeGreaterThan(0);
      const t = res[0];
    ...
    it('should return 5分钟K线', async () => {
      const res = await sdk.kline.cnMinute('sz000858', { period: '5' });
      expect(res.length).toBeGreaterThan(0);
      const k = res[0];
    ...
    it('should return 60分钟K线', async () => {
      const res = await sdk.kline.cnMinute('sh600519', { period: '60' });
      expect(res.length).toBeGreaterThan(0);
      expect(typeof res[0].open).toBe('number');
    });

    it('should return 15分钟K线', async () => {
      const res = await sdk.kline.cnMinute('000001', { period: '15' });
      expect(res.length).toBeGreaterThan(0);
    });

    it('should return 30分钟K线', async () => {
      const res = await sdk.kline.cnMinute('000001', { period: '30' });
      expect(res.length).toBeGreaterThan(0);
    });

    it('should support qfq adjust', async () => {
      const res = await sdk.kline.cnMinute('000001', { period: '5', adjust: 'qfq' });
      expect(res.length).toBeGreaterThan(0);
    });

    it('should support no adjust', async () => {
      const res = await sdk.kline.cnMinute('000001', { period: '5', adjust: '' });
      expect(res.length).toBeGreaterThan(0);
    });

    it('should handle sz prefix', async () => {
      const res = await sdk.kline.cnMinute('sz000001', { period: '5' });
      expect(res.length).toBeGreaterThan(0);
    });

    it('should handle sh prefix', async () => {
      const res = await sdk.kline.cnMinute('sh600000', { period: '5' });
      expect(res.length).toBeGreaterThan(0);
    });

    it('分时应处理带前缀的代码', async () => {
      const res = await sdk.kline.cnMinute('sz000001', { period: '1' });
      expect(res.length).toBeGreaterThan(0);
    });

    it('should filter by time range', async () => {
      const res = await sdk.kline.cnMinute('000001', {
        period: '5',
    ...
    it('should throw for invalid period', async () => {
      await expect(
        sdk.kline.cnMinute('000001', { period: '2' as any })
      ).rejects.toThrow(/period/i);
    });

    it('should throw for invalid adjust', async () => {
      await expect(
        sdk.kline.cnMinute('000001', { period: '5', adjust: 'bad' as any })
      ).rejects.toThrow(/adjust/i);
    });
  });
});
